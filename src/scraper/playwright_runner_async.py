...
# Purpose: Robust async scraper without expect_navigation. Uses selector/URL checks and JSON paging.
...
PAGE_LEN_DEFAULT = 100  # try 100 first; raise to 500 if server permits
...
async def _scrape_datatables_all(
    ...
    async with async_playwright() as p:
        ...
        # 1) Login without any expect_navigation
        ...
        # Evidence-based waits, not navigation waits
        try:
            await page.wait_for_load_state("networkidle", timeout=120_000)
        except PWTimeout:
            pass

        login_ok = "Account/Login" not in page.url
        if not login_ok:
            # Check for elements only visible post-login
            try:
                await page.wait_for_selector('a[href*="Account/Logoff"]', timeout=90_000)
                login_ok = True
            except PWTimeout:
                try:
                    await page.wait_for_selector('a.btn.btn-warning[href*="RiskBookingAllList"]', timeout=90_000)
                    login_ok = True
                except PWTimeout:
                    login_ok = False
        ...
        if not login_ok:
            ...
            await browser.close()
            raise RuntimeError("Login failed or slow. See _diag/login_failed.*")

        # 2) Go to list page directly (no expect_navigation)
...
async def run_and_export_async(
    ...
    base_url: str,
    headless: bool,
    max_pages: int,  # kept for compatibility; unused for JSON paging
    csv_local_path: str,
